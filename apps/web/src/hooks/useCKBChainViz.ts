  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      updateState({
        ...
        pendingTransactions: (snapshotData.pendingTransactions || []).map(tx => ({ ...tx, status: 'PENDING' as const })),
        proposedTransactions: (snapshotData.proposedTransactions || []).map(tx => ({ ...tx, status: 'PROPOSED' as const })),
        confirmedTransactions: (snapshotData.confirmedTransactions || []).map(tx => ({ ...tx, status: 'CONFIRMED' as const })),
