...
/**
 * Gets the primary brand color based on current network mode
 */
const getPrimaryColor = (): string => {
    const network = document.documentElement.getAttribute('data-network');
    if (network === 'testnet') {
        return getComputedStyle(document.documentElement)
            .getPropertyValue('--color-brand-testnet-primary')
            .trim();
    }
    return getComputedStyle(document.documentElement)
...
export function TransactionDetail({
    ...
    // Get primary color based on network
        ...
        >
            <div className="tx-detail-content">
                ...
                <div className="tx-detail-header">
                    {data.animalType ? (
                        <img
                            src={getAnimalIcon(data.animalType)}
                            alt={data.animalType}
                            className="tx-detail-icon-img"
                        />
                    ) : (
                        <div className="tx-detail-icon">🐢</div>
                    )}
                    <div className="tx-detail-hash-section">
                        <div className="tx-detail-hash text-h2">
                        ...
                        <div className="tx-detail-category">
                            <div
                                ...
                                style={{
                                    backgroundColor:
                                        data.category && data.category in TX_TYPE_COLORS
                                            ? TX_TYPE_COLORS[
                                                  data.category as keyof typeof TX_TYPE_COLORS
                                              ]
                                            :'#F8F0DC',
                                }}
                            ></div>
