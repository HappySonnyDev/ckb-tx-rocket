...
export class SyncService implements OnModuleInit {
    ...
    async onModuleInit() {
        this.logger.log(
            'SyncService initialized. Subscribing to CKB events...',
        );
        await Promise.all([
                ...
                async (blockString: string) => {
                    try {
                        ...
                        await this.prisma.$transaction(async (prisma) => {
                            const savedBlock =
                                await this.blockService.upsertBlock(
                                    block,
                                    prisma,
                                );
                                ...
                                const proposedPayload: TransactionProposedPayload =
                                    {
                                        ...
                                        context: {
                                            blockNumber:
                                                savedBlock.number.toString(),
                                            blockHash: savedBlock.hash,
                                        },
                                    };
                                this.eventService.emitTransactionProposed(
                                    proposedPayload,
                                );
                            }
                            ...
                            for (const [
                                ...
                                this.eventService.emitTransactionConfirmed(
                                    confirmedPayload,
                                );
                            }

                            const transactionSummaries = block.transactions.map(
                                (tx) => ({
                                    txHash: tx.hash,
                                }),
                            );
                ...
                async (txEntry: string) => {
                    try {
                        const parsedTx = JSON.parse(
                            txEntry,
                        ) as NewTransactionEntry;
                        await this.transactionService.processPendingTx(
                            parsedTx,
                        );
                        ...
                        this.eventService.emitTransactionPending(
                            pendingPayload,
                        );
    ...
    private async subscribeToProposedTransaction() {
        try {
            ...
            this.logger.error(
                'Failed to subscribe to proposed_transaction',
                error,
            );
        }
    }
    ...
    private async subscribeToRejectedTransaction() {
        try {
            await this.ckbWebsocketService.subscribe(
                ...
                async (txEntry: string) => {
                    try {
                        ...
                        this.eventService.emitTransactionRejected(
                            rejectedPayload,
                        );
            ...
            this.logger.error(
                'Failed to subscribe to rejected_transaction',
                error,
            );
        }
    }
